...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly create loa-tools-test-d184
  fly create loa-tools-test-d184-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app loa-tools-test-d184
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app loa-tools-test-d184-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'loa-tools-test-d184-staging' does not match app name in config file 'loa-tools-test-d184'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name loa-tools-test-d184-db
  fly postgres attach --postgres-app loa-tools-test-d184-db --app loa-tools-test-d184

  fly postgres create --name loa-tools-test-d184-staging-db
  fly postgres attach --postgres-app loa-tools-test-d184-staging-db --app loa-tools-test-d184-staging
  ```
